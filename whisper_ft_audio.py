...
class MuavicSpeechDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, id):
        if len(self.audio_info_list[id]) == 4:
            ...
            # include_audio_lens=True: (lang, audio_path, text, audio_length, video_path)
            lang, audio_path, text, _, _ = self.audio_info_list[id]
...
wandb_logger, checkpoint_callback, callback_list = setup_logging_and_checkpoint(cfg.log_output_dir, 
...
# After training is complete, save the checkpoint with config
import torch
...
# Save checkpoint
checkpoint_path = f"{cfg.check_output_dir}/{cfg.train_id}/final_with_cfg.pth"
torch.save({
    'model_state_dict': model.state_dict(),
    # ... (add other items if needed, e.g. optimizer state)
}, checkpoint_path)
