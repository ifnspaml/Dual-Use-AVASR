...
class AudioEncoder(nn.Module):
    def __init__(
        ...
        if video:
            if av_fusion == "effusion":
                self.video_projection_scalar_early = nn.Parameter(torch.tensor(0.))
                print("DEBUG: video_projection_scalar_early: {}".format(self.video_projection_scalar_early))
            self.video_projection_scalar = nn.Parameter(torch.tensor(1.)) # Note: this is a scalar, not a vector
            print("DEBUG: video_projection_scalar: {}".format(self.video_projection_scalar))
            ...
            if not av_hubert_encoder:
                ...
                print("Using ResEncoder for video model, weights loaded from: {}".format(video_model_path))
            else:
                ...
                print("Using output layer: {} of the AV-HuBERT encoder as visual features".format(self.output_layer))
            ...
            if self.av_fusion == "dual_use_concat":
                self.projection_concat = Linear(n_state*2, n_state)
    ...
    def forward(self, x: Tensor, x_v=None, training=False, test_a=False, test_v=False, track_norm=False, 
        ...
        """
        # NOTE: audio encoder 
        # 1. F^A(x^A) Audio feature extraction with audio frontend
        x_a_original = None
        x_v_original = None
        x_v_flamingo = None
        if not test_v:
        ...
        # Save the original audio features
        # if self.iterative_fusion != 0:
        #    x_a_original = x
        # NOTE: video encoder
        # 2. Iterative Fusion is only with AVHuBERT as video model:
        # 2.1. h^V = E(F(x^V)) Here only initialization for interative fusion.
        if self.video and not test_a:
            if not self.av_hubert_encoder:
                ...
                output = self.video_model(source={'video': x_v, 'audio': None}, padding_mask=padding_mask, mask=False, features_only=True, output_layer=self.output_layer)
                x_v = output['x']
            else:
                x_v = self.video_model(source={'video': x_v, 'audio': None}, padding_mask=padding_mask, output_layer=self.output_layer)
                x_v = x_v['encoder_out'].permute(1, 0 , 2) # T, B, F -> B, T, F
            if track_norm:
                x_v_norm_pre = torch.linalg.norm(x_v, dim=-1).mean()

            # Video projection: project video features to audio feature space
            # 3.1 B^A(h^V) In iterative fusion, repeat video features to match the audio features
            if self.av_fusion == "lip-reader" or self.av_fusion == "effusion":
                x_v = torch.repeat_interleave(x_v, 2, dim=1) # 25 Hz -> 50 Hz 
            x_v = self.video_projection(x_v)
            if self.av_fusion == "effusion":
                x_v_flamingo = self.video_projection_scalar * x_v
                x_v = self.video_projection_scalar_early * x_v
            elif self.av_fusion == "dual_use_concat":
                x_v_flamingo = self.video_projection_scalar * x_v
            else:
                x_v = self.video_projection_scalar * x_v
                x_v_flamingo = x_v

            if self.av_fusion == "lip-reader": # NOTE: positional embedding of lip-reader mode is added to video features
                ...
                if self.av_fusion == "lip-reader": # NOTE: new transformer layers are only used for lip-reader
            ...
            # NOTE: effusion mode: add video features to audio features
            # 3.2 h^A = E^A(F^A(x^A), B^A(h^V)) = F^A(x^A) + B^A(h^V)
            # Add video features to audio features from the audio frontend
            # In iterative fusion, the audio features are initialized
            elif self.av_fusion == "effusion" or self.av_fusion == "dual_use_concat":
                # Zero-pad x or x_v along time dimension to match lengths before addition
                # if x is not None and x_v is not None:
                #     x, x_v = match_time_dimensions(x, x_v)
                if x is not None and x_v is not None:
                    len_x = x.shape[1]
                    len_xv = x_v.shape[1]
                    if len_x != len_xv:
                        if len_x < len_xv:
                            pad_amt = len_xv - len_x
                            x = F.pad(x, (0, 0, 0, pad_amt))  # pad time dim (dim=1)
                        else:
                            pad_amt = len_x - len_xv
                            x_v = F.pad(x_v, (0, 0, 0, pad_amt))  # pad time dim (dim=1)
                if training:
                    mode_drop_prob = np.random.random()
                    if 0 < mode_drop_prob <= self.prob_av:
                        if self.av_fusion == "effusion":
                            x = x + x_v # Note: add video features to audio features
                        elif self.av_fusion == "dual_use_concat":
                            x = self.projection_concat(torch.cat([x, x_v], dim=-1))
                    elif self.prob_av < mode_drop_prob <= self.prob_av + self.prob_a:
                        x = x # Note: use audio features only
                    else:
                        x = x_v # drop audio
                else:
                    if self.av_fusion == "effusion":
        ...
        # NOTE: pos embedding for audio-only mode and audiovisual mode
        if not test_v:
        ...
        # 5. Audio encoder: feed audio/audiovisual features to the audio encoder
        for layer, block in enumerate(self.blocks):
            x = block(x) # NOTE: h^A = E^A(F^A(x^A), B^A(h^V))

        x = self.ln_post(x)

        if training and self.av_fusion != "effusion":  # modality dropout, encoder
            ...
            if 0 < mod_drop_prob <= self.prob_av:
                ...
                x = 0 * x # drop audio
        if track_norm:
            return x, x_norm, x_v_norm_pre, x_v_norm_post, x_v_flamingo
        if self.av_fusion == "effusion" or self.av_fusion == "dual_use_concat":
            return x, x_v_flamingo
        else:
            return x, x_v_flamingo
...
class Whisper(nn.Module):
    def __init__(self, dims: ModelDimensions, dropout_rate: float, video: bool,
                 ...
                 freeze_loaded_params: bool, output_layer: int = None):
        super().__init__()
        self.dims = dims
...
def match_time_dimensions(x_a_original, x_v):
    """
    Adjust x_v so its time dimension matches x_a_original's time dimension.
    If x_v is shorter, pad it; if longer, crop it. x_a_original is never changed.
    Returns x_a_original (unchanged) and the adjusted x_v.
    """
    if x_a_original is not None and x_v is not None:
        len_xa = x_a_original.shape[1]
        len_xv = x_v.shape[1]
        if len_xv < len_xa:
            pad_amt = len_xa - len_xv
            x_v = F.pad(x_v, (0, 0, 0, pad_amt))  # pad time dim (dim=1)
        elif len_xv > len_xa:
            x_v = x_v[:, :len_xa, :]
    return x_a_original, x_v
