...
class ResEncoder(nn.Module):
    def __init__(self, relu_type, weights):
        ...
        if weights is not None and weights != '':
            print(f"Load {weights} for resnet")
            std = torch.load(weights, map_location=torch.device('cpu'))
            if 'ft_' in weights:
                std = {'.'.join(k.split('.')[4:]) : v for k, v in std['model'].items() if "resnet" in k}
            else:
                std = {'.'.join(k.split('.')[2:]) : v for k, v in std['model'].items() if "resnet" in k}
            frontend_std, trunk_std = OrderedDict(), OrderedDict()
            number_loaded_params = 0
            for key, val in std.items():
                new_key = '.'.join(key.split('.')[1:])
                if 'frontend3D' in key:
                    frontend_std[new_key] = val
                    number_loaded_params += val.numel()
                if 'trunk' in key:
                    trunk_std[new_key] = val
                    number_loaded_params += val.numel()
            self.frontend3D.load_state_dict(frontend_std)
            self.trunk.load_state_dict(trunk_std)
            number_resencoder_params = sum(p.numel() for p in self.parameters())
            if number_loaded_params == number_resencoder_params:
                print("Loaded {number_loaded_params} parameters from {weights}", number_loaded_params, weights)
            else:
                print("Loaded {number_loaded_params} parameters from {weights}, but expected {number_resencoder_params}", number_loaded_params, weights, number_resencoder_params)
