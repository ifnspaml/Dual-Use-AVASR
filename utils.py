...
# AV fusion mode: single config that maps to (av_fusion, add_gated_x_attn).
# - baseline: separate streams + gated cross-attn (Whisper-Flamingo baseline).
# - early_fusion: effusion (early add) + no gated cross-attn (proposed early fusion).
# - dual_use: effusion + gated cross-attn (proposed dual-use).
# - dual_use_concat: concat then project (old effusion-concat).
FUSION_MODE_TO_AV_FUSION = {
    "baseline": ("separate", 1),
    "early_fusion": ("effusion", 0),
    "dual_use": ("effusion", 1),
    "dual_use_concat": ("dual_use_concat", 1),
}


def get_av_fusion_params(config):
    """
    Get (av_fusion, add_gated_x_attn) from config. If config has fusion_mode
    (baseline | early_fusion | dual_use), use that; otherwise use
    config.av_fusion and config.add_gated_x_attn for backward compatibility.
    config can be a SimpleNamespace (cfg) or argparse args.
    """
    fusion_mode = getattr(config, "fusion_mode", None)
    if fusion_mode is not None and fusion_mode in FUSION_MODE_TO_AV_FUSION:
        return FUSION_MODE_TO_AV_FUSION[fusion_mode]
    av_fusion = getattr(config, "av_fusion", "effusion")
    add_gated_x_attn = getattr(config, "add_gated_x_attn", 0)
    return (av_fusion, add_gated_x_attn)


def apply_fusion_mode_to_config(config):
    """
    In-place: set config.av_fusion and config.add_gated_x_attn from
    config.fusion_mode if present. Call after loading YAML/args so downstream
    code can keep using cfg.av_fusion and cfg.add_gated_x_attn.
    """
    av_fusion, add_gated_x_attn = get_av_fusion_params(config)
    config.av_fusion = av_fusion
    config.add_gated_x_attn = add_gated_x_attn
...
def load_data(AUDIO_MAX_LENGTH, TEXT_MAX_LENGTH, langs=['en', 'ar', 'de', 'el', 'es', 'fr', 'it', 'pt', 'ru'],
              muavic_root='//beegfs/data/shared/muavic/', reduce_val=None, include_audio_lens=False,
              AUDIO_MAX_LENGTH_VAL=480000, vc2=False, vc2_path='', lrs2=False, visible=False, task='transcribe', data_subset=None):
    ...
    for lang in langs:
        for split in audio_transcript_pair_list:
            if lrs2:
                txt_fn = os.path.join('//beegfs/data/shared/lrs2/lrs2_avhubert/224h_data/{}.wrd'.format(split))
                tsv_fn = os.path.join('//beegfs/data/shared/lrs2/lrs2_avhubert/224h_data/{}.tsv'.format(split))
            elif lang == 'en':
                if split == 'train' and vc2:
                    ...
                    if data_subset == '90h':
                        tsv_fn = os.path.join(muavic_root, 'muavic_10h_filter', 'en', 'muavic_normalized', '{}.tsv'.format(split))
                        txt_fn = os.path.join(muavic_root, 'muavic_10h_filter', 'en', 'muavic_normalized', '{}.en'.format(split))
                    elif data_subset == 'lrs3-30h':
                        tsv_fn = os.path.join('//beegfs/data/shared/lrs3/30h_data', '{}.tsv'.format(split))
                        txt_fn = os.path.join('//beegfs/data/shared/lrs3/30h_data', '{}.wrd'.format(split))
                    elif data_subset == 'lrs23-657h':
                        tsv_fn = os.path.join('//beegfs/work_fast/zhengyangli/av_training_data_scaling_up/lrs23', '{}.tsv'.format(split))
                        txt_fn = os.path.join('//beegfs/work_fast/zhengyangli/av_training_data_scaling_up/lrs23', '{}.en'.format(split))
                    elif data_subset == 'lrs23-vc2':
                        tsv_fn = os.path.join('//beegfs/work_fast/zhengyangli/av_training_data_scaling_up/lrs23_vc2', '{}.tsv'.format(split))
                        txt_fn = os.path.join('//beegfs/work_fast/zhengyangli/av_training_data_scaling_up/lrs23_vc2', '{}.en'.format(split))
                    else:
                ...
                if task == 'transcribe':
                    if split == 'train' and vc2:
                        ...
                        if data_subset == '90h':
                            tsv_fn = os.path.join(muavic_root, 'muavic_10h_filter', lang, 'muavic_normalized', '{}.tsv'.format(split))
                            txt_fn = os.path.join(muavic_root, 'muavic_10h_filter', lang, 'muavic_normalized', '{}.{}'.format(split, lang))
                        else:
            ...
            with open(tsv_fn) as tsv:
                with open(txt_fn) as txt:
                    ...
                    # video path, audio path, video frames, audio length, text, text length
                    wav_fns = [(audio.strip().split('\t')[1], audio.strip().split('\t')[2], int(audio.strip().split('\t')[3]), int(audio.strip().split('\t')[4]), txt.strip(), 
                                len(txt.strip())) for audio, txt in zip(audio_lns, txt_lns)]
                    pre_video_check = len(wav_fns)
                    wav_fns =  list(filter(lambda x: x[2] > 0, wav_fns))  # video frames > 0
                    ...
                    pre_video_check = len(wav_fns)
                    wav_fns =  list(filter(lambda x: x[5] > 0, wav_fns))  # text length > 0
                    post_video_check = len(wav_fns)
                    print("Removed {} samples with missing text".format(pre_video_check - post_video_check))
                    len_before = len(wav_fns)
                    if split == 'train': 
                        wav_fns =  list(filter(lambda x: x[3] <= AUDIO_MAX_LENGTH, wav_fns))  # audio length <= max
                        wav_fns =  list(filter(lambda x: x[5] <= TEXT_MAX_LENGTH, wav_fns))  # text length <= max
                    elif split == 'valid': # whisper pos. embedding only up to 30s long, don't filter test
                        wav_fns =  list(filter(lambda x: x[3] <= AUDIO_MAX_LENGTH_VAL, wav_fns))  # audio length <= max val
                    print("Total hours {} : {}".format(split, sum([int(x[3]) for x in wav_fns]) / 16000 / 3600))
                    if not include_audio_lens:
                        lang_filtered = [(lang, i[1], i[4], i[0]) for i in wav_fns]  # lang, audio_path, text, video_path
                    else: 
                        lang_filtered = [(lang, i[1], i[4], i[3], i[0]) for i in wav_fns]  # lang, audio_path, text, audio_length, video_path
...
def setup_logging_and_checkpoint(log_output_dir, check_output_dir, train_name, train_id, monitor='val/acc'):
    ...
    # callback_list = [checkpoint_callback,
    #                  LearningRateMonitor(logging_interval="step")]
    return wandb_logger, checkpoint_callback, callback_list
