import os
import json
import argparse
import numpy as np
import torch
from torch import nn
from scipy.io import wavfile
import pandas as pd
import whisper
from whisper.normalizers import EnglishTextNormalizer, BasicTextNormalizer
import editdistance
from pytorch_lightning import LightningModule
from pytorch_lightning import Trainer, seed_everything
from tqdm import tqdm
from spec_augment import spec_augment
from utils import (
    load_data,
    WhisperVideoCollatorWithPadding,
    apply_fusion_mode_to_config,
)
from utils_batch_samplers import LengthBatchSampler
#from whisper_ft_av import MuavicVideoDataset
from whisper_ft_av import MuavicVideoDataset
from fairseq.scoring.wer import WerScorer, WerScorerConfig
import sacrebleu

parser = argparse.ArgumentParser()
parser.add_argument('--lang', default='ru', type=str, help='decoding language')
parser.add_argument('--model-type', default='medium', help='Whisper model size, note: large-v2, not large')
parser.add_argument('--noise-snr',  default=1000, type=int, help='>100 is off, so 1000 means clean audio')
parser.add_argument('--noise-fn', default=None, help='testing noise file')
parser.add_argument('--beam-size', default=1, type=int, help='if 1 use greedy else beam search')
parser.add_argument('--modalities', default="avsr", help='asr for audio-only, avsr for audio-visual')
parser.add_argument('--use_av_hubert_encoder', default=0, type=int, help='if 1 use av hubert encoder')
parser.add_argument('--av_fusion', default="", help='Legacy: effusion|separate|effusion-concat (old checkpoints). Prefer --fusion-mode for new configs.')
parser.add_argument('--fp16', default=1, type=int, help='if 1 use fp16, if 0 use GPU if available or cpu if not')
parser.add_argument('--checkpoint-path', default=None, help='path to load the checkpoint from')
parser.add_argument('--decode-path', default="decode/", help='path to save the decode results')
parser.add_argument('--whisper-path', default="models/", help='path to download OpenAI whisper weights')
parser.add_argument('--av-hubert-path', default="av_hubert/avhubert/", help='path to avhubert code')
parser.add_argument('--av-hubert-ckpt', default="models/large_noise_pt_noise_ft_433h_only_weights.pt", 
                                        help='path to avhubert ckpt (needed to load the model architecture)')
parser.add_argument('--task', default='transcribe', type=str, help='transcribe, En-X, X-En')
parser.add_argument('--normalizer', default='fairseq', type=str, help='whisper OR fairseq')
parser.add_argument('--use-original-whisper', default=0, type=int, 
                                        help='if 1, ignore checkpoint-path and use original whisper')
parser.add_argument('--add-gated-x-attn', default=0, type=int, help='if 1 use gated x attn (legacy)')
parser.add_argument('--fusion-mode', dest='fusion_mode', default=None,
                    choices=['baseline', 'early_fusion', 'dual_use', 'dual_use_concat'],
                    help='New: AV fusion mode (overrides av_fusion when set). If checkpoint has config.yaml with fusion_mode, that is used instead.')
parser.add_argument('--split', default='test', type=str, choices=['test', 'valid'],
                    help='Which split to decode: test or valid')
args = parser.parse_args()
if args.fusion_mode is not None:
    apply_fusion_mode_to_config(args)
SAMPLE_RATE = 16000
SEED = 3407
seed_everything(SEED, workers=True)

# Select split
decode_split = args.split if args.split in ['test', 'valid'] else 'test'

# Ensure add_gated_x_attn exists in args
if not hasattr(args, 'add_gated_x_attn'):
    args.add_gated_x_attn = 0

# process lang and task
visible = True if 'visible' in args.lang else False
if 'visible' in args.lang:
    args.lang = args.lang.replace('-visible', '')
use_lrs2 = True if args.lang == 'lrs2' else False
if args.lang == 'lrs2':
    args.lang = 'en'

# audio_transcript_pair_list = load_data(480000, 350, [args.lang], muavic_root='/data/sls/scratch/roudi/datasets/muavic/', 
audio_transcript_pair_list = load_data(480000, 350, [args.lang], muavic_root='//beegfs/data/shared/muavic/', 
                                       include_audio_lens=True, task=args.task, lrs2=use_lrs2)

# Select the correct split
dataset_split =  audio_transcript_pair_list[decode_split]
# print first 20 elements of dataset_split
# print(f"DEBUG: dataset_split[:20] before replace: {dataset_split[:20]}")
#dataset_split = [[i[0], i[1].replace('/data/sls/scratch/roudi/datasets/muavic/', ''),
#                                    i[2], i[3]] for i in dataset_split] # fix paths
#print(f"DEBUG: dataset_split[:20] after replace: {dataset_split[:20]}")
# test_dataset = [[i[0], i[1], i[2], i[3]] for i in test_dataset] # use original paths
multilingual = True if 'large' in args.model_type or 'en' not in args.model_type else False
# print("Multilingual tokenizer : {}".format(multilingual))

# We use the transcribe token (not translate) for En-X to enable new capabilities
task = 'translate' if args.task == 'X-En' else 'transcribe'
tokenizer = whisper.tokenizer.get_tokenizer(multilingual=multilingual, task=task) 
special_token_set = set(tokenizer.special_tokens.values())

args.checkpoint_path= None if args.use_original_whisper else args.checkpoint_path
# If the original Whisper from OpenAI is used, crop / pad the audio to 30s
dataset = MuavicVideoDataset(dataset_split, 
                                tokenizer, 
                                SAMPLE_RATE, 
                                args.model_type,
                                max_length=None if args.checkpoint_path else SAMPLE_RATE * 30,
                                spec_augment="", # no spec augment
                                noise_prob=1 if args.noise_snr != 1000 else 0,
                                noise_fn = args.noise_fn,
                                train=False, # video center crop, no flip
                                noise_snr=args.noise_snr,)   

# For beam size of 1, use batch decoding with ~40s of audio per batch
# For beam size >1, each audio sample is decoded separately
length_sorter = LengthBatchSampler(batch_bins=SAMPLE_RATE * 40 if args.checkpoint_path and \
                                   args.beam_size == 1 else 1,
                            shapes=[i[3] for i in dataset_split],
                            sort_in_batch='descending',
                            sort_batch='descending',
                            drop_last=False)

dataloader = torch.utils.data.DataLoader(dataset,
                    num_workers=8,
                    collate_fn=WhisperVideoCollatorWithPadding(),
                    batch_sampler=length_sorter)

def load_whisper_model_from_checkpoint_or_args(args):
    """
    Loads the whisper model from checkpoint (with config if available),
    or constructs it from args if no config is found.
    Returns the constructed whisper_model.
    """
    if args.checkpoint_path is not None and not args.use_original_whisper:
        print("Loading checkpoint and config (if available)")
        checkpoint = torch.load(args.checkpoint_path, map_location=torch.device('cpu'))
        # Load config from YAML file
        config_path = f"{args.checkpoint_path.rsplit('/', 1)[0]}/config.yaml"
        try:
            with open(config_path, 'r') as config_file:
                import yaml
                cfg = yaml.safe_load(config_file)
                import types
                cfg = types.SimpleNamespace(**cfg)
                apply_fusion_mode_to_config(cfg)
                print("Loading config from YAML:")
                print(cfg)
        except FileNotFoundError:
            print("Config YAML not found, falling back to checkpoint config.")
            if 'cfg' in checkpoint:
                cfg = checkpoint['cfg']
                import types
                cfg = types.SimpleNamespace(**cfg)
                print("Loading config from checkpoint:")
                print(cfg)
            else:
                print("No config found in checkpoint, falling back to argument-based model loading.")
                cfg = None
        if cfg is not None:
            apply_fusion_mode_to_config(cfg)
        
        def get_state_dict(ckpt):
            if 'model_state_dict' in ckpt:
                return ckpt['model_state_dict']
            elif 'state_dict' in ckpt:
                sd = ckpt['state_dict']
                # Remove 'model.' prefix if present
                new_sd = {}
                for k, v in sd.items():
                    if k.startswith('model.'):
                        new_sd[k[6:]] = v  # Remove 'model.' prefix
                    else:
                        new_sd[k] = v
                return new_sd
            else:
                raise KeyError("No model state dict found in checkpoint (tried 'model_state_dict' and 'state_dict')")
                # Load the model using the loaded config.
        # For audio-only decode (av_fusion "None"), build audio-only model so we don't need
        # ResNet/AV-HuBERT weights; checkpoint state_dict will be loaded with strict=False.
        decode_audio_only = (args.av_fusion == "None" or getattr(args, 'modalities', '') == 'asr')
        if decode_audio_only:
            use_video = False
            av_fusion_val = "None"
            video_model_path_val = ""   # not used when video=False
            av_hubert_path_val = ""    # not used when video=False
            av_hubert_encoder_val = 0
        else:
            use_video = bool(cfg and hasattr(cfg, 'av_fusion') and getattr(cfg, 'av_fusion', "None") != "None")
            av_fusion_val = getattr(cfg, 'av_fusion', args.av_fusion) if cfg else args.av_fusion
            video_model_path_val = getattr(cfg, 'av_hubert_ckpt', args.av_hubert_ckpt) if cfg else args.av_hubert_ckpt
            av_hubert_path_val = getattr(cfg, 'av_hubert_path', args.av_hubert_path) if cfg else args.av_hubert_path
            av_hubert_encoder_val = getattr(cfg, 'use_av_hubert_encoder', 0) if cfg else args.use_av_hubert_encoder
        whisper_model = whisper.load_model(
            cfg.model_name if cfg else args.model_type,
            device=None,
            download_root=cfg.download_root if cfg and hasattr(cfg, 'download_root') else args.whisper_path,
            in_memory=False,
            dropout_rate=cfg.dropout_rate if cfg else 0.0,
            video=use_video,
            video_model_path=video_model_path_val,
            av_hubert_path=av_hubert_path_val,
            prob_av=getattr(cfg, 'prob_av', 0.0) if cfg else 0.0,
            prob_a=getattr(cfg, 'prob_a', 0.0) if cfg else 0.0,
            av_hubert_encoder=av_hubert_encoder_val,
            av_fusion=av_fusion_val,
            add_adapter=getattr(cfg, 'add_adapter', False) if cfg else False,
            adapter_dim=getattr(cfg, 'adapter_dim', 256) if cfg else 256,
            add_gated_x_attn=0 if decode_audio_only else (getattr(cfg, 'add_gated_x_attn', 1 if getattr(cfg, 'av_fusion', args.av_fusion) == 'separate' else 0) if cfg else 0),
            freeze_loaded_params=cfg.freeze_loaded_params if cfg else False,
            output_layer=getattr(cfg, 'output_layer', None) if cfg else None,
        )
        print("Model architecture:\n", whisper_model)
        print("Total parameters: {:,}".format(sum(p.numel() for p in whisper_model.parameters())))
        # print the number of parameters in the whisper_model with three levels deep
        for name, module in whisper_model.named_modules():
            # Count the number of dots to determine depth
            depth = name.count('.')
            if depth <= 3:  # Only show up to 3 levels deep (0, 1, 2 dots)
                param_count = sum(p.numel() for p in module.parameters())
                if param_count > 0:  # Only show modules with parameters
                    indent = "  " * depth
                    print(f"{indent}{name}: {param_count:,}")
        print("Loading model weights from checkpoint")
        state_dict = get_state_dict(checkpoint)
        try:
            whisper_model.load_state_dict(state_dict)
        except BaseException as e:
            print(str(e))
            print("Loading weights with strict=False")
            whisper_model.load_state_dict(state_dict, strict=False)
        return whisper_model

    else:
        print("Loading Whisper")
        whisper_model = whisper.load_model(
            args.model_type,
            device=None,
            download_root=args.whisper_path,
            in_memory=False,
            dropout_rate=0.0,
            video=True if args.av_fusion != "None" else 0,
            video_model_path=args.av_hubert_ckpt,
            av_hubert_path=args.av_hubert_path,
            prob_av=0.0,
            prob_a=0.0,
            av_hubert_encoder=args.use_av_hubert_encoder,
            av_fusion=args.av_fusion,
            add_adapter=False,
            adapter_dim=256,
            add_gated_x_attn=1 if args.av_fusion == 'separate' else 0,
        )
        print("Model architecture:\n", whisper_model)
        print("Total parameters: {:,}".format(sum(p.numel() for p in whisper_model.parameters())))
        return whisper_model

# Replace the in-line model loading logic with:
whisper_model = load_whisper_model_from_checkpoint_or_args(args)

options = whisper.DecodingOptions(task=task, language=args.lang, fp16=args.fp16, without_timestamps=True, 
                                  beam_size=1 if args.beam_size == None else args.beam_size,)

if args.checkpoint_path is not None:
    out_path = '{}/{}/{}/{}/{}/snr-{}/visible-{}/beam-{}/{}' \
                .format(args.decode_path,args.checkpoint_path, args.lang, decode_split, args.modalities, args.noise_snr, int(visible), 
                        args.beam_size, args.noise_fn.split('/')[-2])
else:
    out_path = '{}/{}/{}/{}/{}/snr-{}/visible-{}/beam-{}/{}' \
                .format(args.decode_path,args.model_type,args.lang, decode_split, args.modalities, args.noise_snr, int(visible),
                        args.beam_size, args.noise_fn.split('/')[-2])
os.makedirs(out_path, exist_ok=True)

# Print relevant args for debugging
print(f"fp16: {args.fp16}, add_gated_x_attn: {args.add_gated_x_attn}, av_fusion: {args.av_fusion}, use_av_hubert_encoder: {args.use_av_hubert_encoder}")

# Convert new paramters to fp16
if (args.fp16 and args.use_av_hubert_encoder) or (args.fp16 and args.av_fusion == 'effusion') or (args.fp16 and args.av_fusion == 'separate' and args.use_av_hubert_encoder == 0) or (args.fp16 and args.av_fusion == 'dual_use_concat'):
    # Convert specific components that need special handling first
    if args.use_av_hubert_encoder or args.av_fusion == 'effusion' or (args.add_gated_x_attn != 0):
        if hasattr(whisper_model.encoder, 'video_projection_scalar'):
            whisper_model.encoder.video_projection_scalar.half()
        if hasattr(whisper_model.encoder, 'video_model'):
            whisper_model.encoder.video_model.half()
            print("DEBUG: video_model.half() is conducted")
    if args.av_fusion == 'dual_use_concat':
        if hasattr(whisper_model.encoder, 'projection_concat'):
            whisper_model.encoder.projection_concat.half()
    
    model_to_num_layers = {'tiny': 4, 'tiny.en': 4, 'base': 6, 'base.en': 6, 'small': 12, 'small.en': 12, 'medium': 24, 'medium.en': 24, 'large-v2': 32}
    if args.av_fusion == 'separate':
        for i in range(model_to_num_layers[args.model_type]):
            try:
                whisper_model.decoder.blocks[i].attn_gate.data = whisper_model.decoder.blocks[i].attn_gate.half()
                whisper_model.decoder.blocks[i].ff_gate.data = whisper_model.decoder.blocks[i].ff_gate.half()
                if hasattr(whisper_model.decoder.blocks[i], 'gated_x_attn'):
                    whisper_model.decoder.blocks[i].gated_x_attn.to(torch.float16)
                    whisper_model.decoder.blocks[i].gated_x_attn_ln.to(torch.float16)
                    whisper_model.decoder.blocks[i].ff_ln.to(torch.float16)
                    whisper_model.decoder.blocks[i].ff.to(torch.float16)
            except:
                continue
    
    # Convert model parameters to FP16
    for param in whisper_model.parameters():
        param.data = param.data.half()
    
    # Convert LayerNorm weights and biases to FP32 (they need to be in FP32 for numerical stability)
    for name, module in whisper_model.named_modules():
        if 'video_model' in name:
            continue
        if isinstance(module, nn.LayerNorm):
            if module.weight is not None:
                module.weight.data = module.weight.data.float()
            if module.bias is not None:
                module.bias.data = module.bias.data.float()

if args.checkpoint_path is not None:
    print("Loading checkpoint")
    state_dict = torch.load(args.checkpoint_path, map_location=torch.device('cpu'))
    print(state_dict.keys())
    state_dict = state_dict['state_dict']
    state_dict_updated = {k[6:]: v  for k, v in state_dict.items()} # remove 'model.'
    try: # newer models have learnable scaler init 1
        whisper_model.load_state_dict(state_dict_updated) 
    except BaseException as e:
        print(str(e))
        print("Loading weights with strict=False")
        whisper_model.load_state_dict(state_dict_updated, strict=False)

hypo, refs = [], []
whisper_model.eval() # AV-HuBERT batch norm and dropout
with open(os.path.join(out_path, 'pred.txt'), 'w+') as f:
    for i, b in enumerate(tqdm(dataloader)):
        if args.fp16:
            input_ids = b["input_ids"].half().cuda()
            video = b["video"].half().cuda()
        else:
            if torch.cuda.is_available():
              input_ids = b["input_ids"].cuda()
              video = b["video"].cuda()
            else:
              input_ids = b["input_ids"]
              video = b["video"]
        labels = b["labels"]
        with torch.no_grad():
            # NOTE: haven't implemented padding mask for AV-HuBERT, but it seems to work fine without it
            if args.modalities == "avsr":
                results = whisper_model.decode(input_ids, options, video)
            elif args.modalities == "asr": 
                results = whisper_model.decode(input_ids, options, video, test_a=True)
            elif args.modalities == "vsr": 
                results = whisper_model.decode(input_ids, options, video, test_v=True)
            else:
                raise NotImplementedError
            
            for r, l in zip(results, labels):
                hypo.append(r.text)
                print('HYPO: {}'.format(r.text))
                f.write('HYPO: {}\n'.format(r.text))

                l[l == -100] = tokenizer.eot
                ref = tokenizer.decode([t for t in l if t.item() not in special_token_set])
                refs.append(ref)
                print('REF: {}'.format(ref))
                f.write('REF: {}\n'.format(ref))

if args.lang == 'en' or args.task == 'transcribe':
    if args.normalizer == 'whisper':
        if args.lang == 'en':
            std = EnglishTextNormalizer()
        else:
            std = BasicTextNormalizer()
        c_err, c_len, w_err, w_len = 0, 0, 0, 0
    else:
        scorer = WerScorer(
            WerScorerConfig(
                wer_tokenizer="13a",
                wer_remove_punct=True,
                wer_char_level=False,
                wer_lowercase=True
            )
        )
    with open(os.path.join(out_path, 'wer.368862'), 'w+') as f:
        for h, r in zip(hypo, refs):
            if args.normalizer == 'whisper':
                w_err += editdistance.eval(std(r).split(), std(h).split())
                w_len += len(r.split())
            else: 
                scorer.add_string(ref=r, pred=h)
                wer = scorer.score()
        if args.normalizer == 'whisper':
            wer = 100. * w_err/w_len
        print("WER: %.4f" % wer)
        f.write("WER: %.4f\n" % wer)
    with open(os.path.join(out_path, 'wer.json'), 'w+',) as fp:
        json.dump({'pred': hypo, 'refs': refs}, fp)
else:
    with open(os.path.join(out_path, 'bleu.368862'), 'w+') as f:
        bleu = sacrebleu.corpus_bleu(hypo, [refs]) #NOTE: [ref] not ref
        print("BLEU: %.4f" % bleu.score)
        f.write("BLEU: %.4f\n" % bleu.score)
    with open(os.path.join(out_path, 'bleu.json'), 'w+',) as fp:
        json.dump({'pred': hypo, 'refs': refs}, fp)
